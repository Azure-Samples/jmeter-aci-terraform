...
resource "azurerm_container_group" "jmeter_workers" {
  ...
  container {
    ...
    commands = [
      "/bin/sh",
      "-c",
      "cp -r /jmeter/* .; /entrypoint.sh -s -J server.rmi.ssl.disable=true -p /jmeter/configs/${var.JMETER_ENVIRONMENT_FILE} -q /jmeter/configs/${var.JMETER_LOAD_CONFIG_FILE}",
    ]
  }
}
...
resource "azurerm_container_group" "jmeter_controller" {
  ...
  container {
    ...
    commands = [
      "/bin/sh",
      "-c",
      "cd /jmeter; /entrypoint.sh -n -J server.rmi.ssl.disable=true -p /jmeter/configs/${var.JMETER_ENVIRONMENT_FILE} -q /jmeter/configs/${var.JMETER_LOAD_CONFIG_FILE} -t ${var.JMETER_JMX_FILE} -l ${var.JMETER_RESULTS_FILE} -e -o ${var.JMETER_DASHBOARD_FOLDER} -R ${join(",", "${azurerm_container_group.jmeter_workers.*.ip_address}")}",
    ]
  }
}
